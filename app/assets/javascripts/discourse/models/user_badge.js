...
Discourse.UserBadge = Discourse.Model.extend({
  ...
  **/
  revoke: function() {
    return Discourse.caughtAjax("/user_badges/" + this.get('id'), {
      type: "DELETE"
    });
  }
});
...
Discourse.UserBadge.reopenClass({
  ...
  **/
  findByUsername: function(username) {
    return Discourse.caughtAjax("/user_badges.json?username=" + username).then(function(json) {
  ...
  **/
  grant: function(badgeId, username) {
    return Discourse.caughtAjax("/user_badges", {
      type: "POST",
      data: {
        username: username,
        badge_id: badgeId
