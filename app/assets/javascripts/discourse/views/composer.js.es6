...
var ComposerView = Discourse.View.extend(Ember.Evented, {
  ...
  ensureMaximumDimensionForImagesInPreview: function() {
    ...
    var style = Discourse.Mobile.mobileView ?
                ...
                'max-width:' + this.get('siteSettings.max_image_width') + 'px;' +
                'max-height:' + this.get('siteSettings.max_image_height') + 'px;';
  ...
  titleValidation: function() {
    ...
    } else if (titleLength > this.get('siteSettings.max_topic_title_length')) {
      reason = I18n.t('composer.error.title_too_long', { max: this.get('siteSettings.max_topic_title_length') });
    }

    if (reason) {
      return Discourse.InputValidation.create({ failed: true, reason: reason });
    }
  }.property('model.titleLength', 'model.missingTitleCharacters', 'model.minimumTitleLength', 'siteSettings.max_topic_title_length'),

  categoryValidation: function() {
    if (!this.get('siteSettings.allow_uncategorized_topics') && !this.get('model.categoryId')) {
      return Discourse.InputValidation.create({ failed: true, reason: I18n.t('composer.error.category_missing') });
    }
  }.property('model.categoryId', 'siteSettings.allow_uncategorized_topics'),
